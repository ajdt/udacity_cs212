...
import string # to access string.lowercase and string.uppercase
...
# extra data members and functions added by Armando to accomodate
# blank tiles
blank_tile = '_'
def is_prefix(str): return str.upper() in PREFIXES
def is_word(str): return str.upper() in WORDS
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    i = start + len(pre)
    if is_word(pre) and anchored and not is_letter(row[i]):
            ...
            for L in hand:
                if L in possibilities:
                    ...
                    for L in possibilities:
                        add_suffixes(hand.replace(blank_tile, '', 1), pre+L.lower(), start, row, results)
...
def find_prefixes(hand, pre='', results=None):
    ...
    if is_word(pre) or is_prefix(pre): results.add(pre)
    if is_prefix(pre) :
        for L in hand:
            if L == blank_tile:
                for l in string.lowercase:
                    find_prefixes(hand.replace(blank_tile, '', 1), pre+l, results)
            else :
...
def calculate_score(board, pos, direction, hand, word):
    ...
    for (n, L) in enumerate(word):
        ...
        letter_points = 0 if L in string.lowercase else POINTS[L]
        total += letter_points * letter_mult
