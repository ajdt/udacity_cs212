...
def bsuccessors2(state):
    ...
    h_op, t_op, selection_set, arrow = get_set_operations(*state)
    return dict((( h_op(frozenset([a,b, 'light'])),
                      t_op(frozenset([a, b, 'light'])) ),
                     (a, b, arrow))
                    for a in selection_set
                    for b in selection_set)
    
def get_set_operations( here, there, key='light' ):
    if key in here:
        return (lambda fset: here - fset, lambda fset: there | fset, here - set(['light']), '->')
    else:
        return (lambda fset: here | fset, lambda fset: there - fset, there - set(['light']), '<-')
