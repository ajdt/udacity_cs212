...
def parse(start_symbol, text, grammar):
    ...
    def parse_atom(atom, text):
        if atom in grammar:  # Non-Terminal: tuple of alternatives
            ...
            print "reg ex is : ", tokenizer % atom
...
JSON = grammar("""string => \" chars \"
chars => char chars | char
char => \"  | \/ | \b | \f | newline | \r | \t | \u four-hex-digits | [A-Za-z]
four-hex-digits => [A-Fa-f0-9]{4}
number => int frac exp | int frac | int exp | int
int => digit-no-zero digits | - digit-no-zero digits | - digit | digit
digit-no-zero => [1-9]
frac => \. digits
exp => e digits
...
value => string | number | object | array | true | false | null""", whitespace='\s*')
...
# current setup doesn't allow \\ as a char, fix it!!!!
print test()
