...
def boggle_words(board, minlength=3):
    ...
    used_squares = [False]*len(board)
    results = set()
    for (i, square) in enumerate(board):
        if square in LETTERS : # square is a letter
            boggle_search(board, used_squares, minlength, i, results, square)
    return results

# NORVIG'S solution is similar to mine except he declares
# his helper function inside of the main boggle function
# allowing him to avoid passing minlength, board and other
# repeated parameters to the helper function

def boggle_search(board, used_squares, minlength, start, results, pre=""):
    "returns a set of all possible words of min-length or greater formed from start"
    used_squares[start]= True
    if pre in WORDS and len(pre) >= minlength:
        results.add(pre)
    if pre in PREFIXES:
        adjacent_squares = neighbors(start, size(board) )
        for sq in adjacent_squares:
            if not used_squares[sq] and board[sq] in LETTERS:
                boggle_search(board, used_squares, minlength, sq, results, pre+board[sq])
    used_squares[start] = False
    return results
