...
def alphabetize(phrase):
    " returns a phrase with words reordered in lexicographical order"
...
def anagrams(phrase, shortest=2):
    ...
    results = set()
    def anagram_helper(letters, shortest, results, pre="", phrase=""):
        " a recursive function to find the anagrams formed with argument letters"
        if not letters and phrase and not pre:
            results.add(alphabetize(phrase))
        if pre in PREFIXES:
            for l in letters:
                anagram_helper(removed(letters, l), shortest, results, pre+l, phrase)
        if pre in WORDS and len(pre) >= shortest:
            anagram_helper(letters, shortest, results, "", phrase + " " + pre)
        return results

    # we must remove all whitespace in phrase, otherwise they will
    # be used to form words!!!
    return anagram_helper(phrase.replace(' ', ''), shortest, results) - set([phrase])
