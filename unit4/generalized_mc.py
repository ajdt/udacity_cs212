...
def mc_problem2(start=(3, 3, 1, 0, 0, 0), goal=None):
    if goal is None :
        goal = (0,0,0)+start[:3]
    goal_set = set([goal])
    return shortest_path_search(start, csuccessors, lambda state: state in goal_set ) # <== insert arguments here
...
# what's the point of the first two lines if goal is
# never used in the first place. I disagree with
# the all_gone function as a decider for reaching our
# goal
