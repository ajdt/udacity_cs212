...
def compute_prefixes(hand, pre='', results=None):
...
def find_prefixes(hand, pre='', results=None):
    """ stores the last hand it was called with
    to speed up calls made on same input consecutively"""
    if not (hand == prev_hand) :
        prev_results = compute_prefixes(hand, pre, results)
    return prev_results
...
def find_prefixes(hand, pre='', results=None):
    ...
    # we know we're at a top level call b/c this is
    # the only time pre == '', for recursive calls
    # pre isn't empty
...
# I should have actually looked at the code for find_prefixes
# more keenly instead of just thinking about the solution
# in my head. Perhaps I would have realized what Norvig did
