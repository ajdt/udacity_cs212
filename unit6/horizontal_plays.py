...
def horizontal_plays(hand, board):
    ...
    for (j, row) in enumerate(board[1:-1], 1):
        ...
        for play in row_plays(hand, row):
            i, word = play
            results.add( ((i,j), word) )


        # I wasn't sure whether the inner tuple
        # should be (i,j) or (j, i) b/c I don't
        # know which entry represents row and
        # which represents column

        # Norvig made his one line more concise
