...
def boggle_words(board, minlength=3):
    ...
    for (i, square) in enumerate(board):
        if square in LETTERS : # square is a letter
            used_squares[i] = True
            results |= boggle_search(board, used_squares, minlength, i, square)
            used_squares[i] = False
    return results

def boggle_search(board, used_squares, minlength, start, pre):
    ...
    if pre in PREFIXES:
        ...
        for sq in adjacent_squares:
            if not used_squares[sq] and board[sq] in LETTERS:
                used_squares[sq] = True
                results |= boggle_search(board, used_squares, minlength, sq, pre+board[sq])
                used_squares[sq] = False
    return results
