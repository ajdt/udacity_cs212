...
def play_score(play) :
    "Return the score for a play"
    score, _, _, _ = play
    return score

# My solution, requires the above function, which
# I wrote for best_play
def best_play(hand, board):
    "Return the highest-scoring play.  Or None."
    sorted_plays = sorted(all_plays(hand, board), key=play_score, reversed=True)
    if not sorted_plays:
        return None
    else :
...
# we don't have to specify a key b/c score is the first
# element of the tuple, so it's used by default in sorting

# remember to take advantage of language constructs,
# like Norvig used
# an if statement within his return statement code
