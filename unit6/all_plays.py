...
def all_plays(hand, board):
    ...
    results = set()
    ###Your code here.
    for ( location, word) in hplays :
        results.add( (location, 'ACROSS', word) )
    for ( (i,j), word) in vplays:
        results.add( ((j,i), 'DOWN', word) )
    return results
    ...
    ACROSS, DOWN = (1, 0), (0,1) # directions word can go
    # going across means incrementing one at a time
    # in the i direction and not in the j direction
    # going down means incrementing in the j direction
    # but not the i direction
