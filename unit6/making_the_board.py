...
BONUS_SYMBOLS = DW, TW, DL, TL = '23:;'

def show(board):
    "Print the board."
    for (i, row) in enumerate(board) :
        for (j, sq) in enumerate(row):
            if BONUS[i][j] in BONUS_SYMBOLS and not is_letter(sq):
                print BONUS[i][j],
            else :
                print sq,
        print

# Norvig's solution is the same except he handles print
# as one line
#    for (i, row) in enumerate(board) :
#        for (j, sq) in enumerate(row):
#            print (sq if is_letter(sq) or sq == '|' else BONUS[i][j]),
#        print
