...
def add_suffixes(hand, pre, results):
    ...
    suffixes = prefixes(hand) + [hand]
    for suffix in suffixes:
        if (pre+suffix) in WORDS:
            results.add(pre+suffix)
    return results

# a simple solution was to simply call extend_prefix
# which already finds all the words possible
#       extend_prefix(pre, letters, result)

# Norvig doesn't want to do this b/c we will want to
# change add_suffixes later to account for collisions

# Norvig's solution:

def add_suffixes_norvig(hand, pre, results):
    """Return the set of words that can be formed by extending pre with letters in hand."""
    if pre in WORDS :
        results.add(pre)
    if pre in PREFIXES:
        for L in hand:
            add_suffixes_norvig(hand.replace(L, '', 1), pre+L, results)
    return results
